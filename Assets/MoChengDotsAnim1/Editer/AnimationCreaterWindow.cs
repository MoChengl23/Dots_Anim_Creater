...
public class AnimationCreaterWindow : EditorWindow
{
    ...
    [MenuItem("Window/DOTS_UnitCreater")]
    private static void ShowWindow()
    {
        var window = GetWindow<AnimationCreaterWindow>();
        window.titleContent = new GUIContent("DOTS_UnitCreater");
        window.Show();
    }
    ...
    private void OnGUI()
    {
        ...
        if (prefab == null)
        {
            EditorGUILayout.LabelField("please add prefab");
            return;
           
        }

        EditorGUILayout.LabelField("____________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________");
        ...
        if (prefab != null)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                SpawnDir = EditorGUILayout.ObjectField("SpawnDir", SpawnDir, typeof(Object), true) as Object;
            }

            if (GUILayout.Button("生成", GUILayout.Height(30)))
            {
                if(SpawnDir == null){
                     EditorUtility.DisplayDialog("Failed", "please add Output dir", "OK" );
                    return;
                }
                ...
                var spawnDir = AssetDatabase.GetAssetPath(SpawnDir);

                UnityEditor.AssetDatabase.CreateAsset(unit, $"{spawnDir}/{prefab.name}.asset");
                FinalBuild();
                unit.mainMaterial = mat ;
                foreach (var i in createdAnimations)
                {
                    ...
                    if (unit.defaultAnimation == null) unit.defaultAnimation = i;
                }
                 EditorUtility.DisplayDialog("Succeed", $"result:  {unit.name}\n store in \n {spawnDir}", "OK" );
    ...
    private void FinalBuild()
    {
        ...
        foreach (var anim in animationStuffs)
        {
            ...
            temp.renderIndexs = new List<int>() { 0 };
    ...
    public class AnimationStuff
    {
        ...
        public void CompleteBake(int a = 1, params object[] parameters)
        {
            ...
            {
                bakeTextures[t].name = string.Format($"{animationName}_{t}");
